...
type TrialBalanceViewProps = {
  compact?: boolean; // If true, render in compact mode for modal
};
...
export function TrialBalanceView({ compact = false }: TrialBalanceViewProps) {
  ...
  // Export to Excel
  function handleExportTb() {
    if (useSeries && seriesData) {
      // Export series data
      const rows = seriesData.rows.map((row) => ({
        Account: row.account_name,
        ...Object.fromEntries(
          seriesData.columns.map((col) => [col, row.values[col] ?? 0])
        ),
      }));
      exportRowsToXlsx(`TB_${from}_${to}`, "Trial Balance", rows);
    } else {
          ...
          <>
            ...
            <div className="flex gap-2">
              <button
                ...
                title={!raw && !seriesData ? "Load the TB first" : "Export Trial Balance to Excel"}
              >
                Export TB (Excel)
              </button>
              <button
